...
if(isset($_REQUEST['data'])!=''){	
	...
	if($_REQUEST['session'] != $_SESSION["_sessiondtm"]){
		...
		header("location:address-print-tamil.php");			
		die();
	}
}
...
<head>
    ...
    <title><?php echo PAGE_TITLE; ?> - Print Tamil Address </title>
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-md-7 align-self-center text-right">
                        <div class="d-flex justify-content-end align-items-center">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Print Tamil Address</li>
                            </ol>
                        </div>
                    </div>
                </div>
                <div class="card">
					...
					<div class="card-body">
						<div class="row">
							<div class="col-md-5 align-self-center">
								<h4 class="card-title">Print Tamil Address</h4>
							</div>
							<div class="col-md-7 align-self-center text-right">
								<div class="d-flex justify-content-end align-items-center">
											...
											if ($pdoRes->rowCount() > 0){											
												...
												while($obj=$pdoRes->fetch()){
													if($cityid != $obj->memb_city){
														...
														$citytamil .= $obj->city_tamil.', ';
														$cityid = $obj->memb_city;
													}													
													if($Sno == 0){
														echo '<div class="'.$break_class.'" width="100%" style="min-height:980px;">';
														//echo $header;
														echo '<table class="table table-bordered table-address-'.$clms.'">';
														echo '<tr><th colspan="'.$clms.'">
													...
													echo '<td class="height200" width="'.$clmWidth.'%"><span class="font-weight-bold">'.$obj->memb_fname_tamil.'. '.$obj->memb_initial_tamil.'</span><span class="memb-id">'.$obj->memb_code.'</span>'.$memb_lname.$memb_add1.$memb_add2.'																
																<br>'.$obj->city_tamil.$memb_pincode.$memb_mobile.'
															</td>';
													$Sno++;
													if ($Sno % $maxrows == 0 && $Sno > 0){
														echo '</tr></tbody>';
														echo '<tr><td colspan="'.$clms.'" align="center"><span class="font-weight-bold">'.substr($cityname, 0, -2).'</span></td></tr>';
														echo '</table></div>';
		...
		$("#resultForm").on("submit", function(event) {
			...
			$.ajax({
				...
				success:function(result){
					//console.log(result);
					window.location.href = "address-print-tamil.php?data="+result;
				}
			});
		});
