...
if(isset($_REQUEST['data'])!=''){	
	...
	if($_REQUEST['session'] != $_SESSION["_sessiondtm"]){
		...
		header("location:address-photo-print.php");			
		die();
	}
}
...
<head>
    ...
    <title><?php echo PAGE_TITLE; ?> - Print Address with Photo</title>
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-md-7 align-self-center text-right">
                        <div class="d-flex justify-content-end align-items-center">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Print Address with Photo</li>
                            </ol>
                        </div>
                    </div>
                </div>
                <div class="card">
					...
					<div class="card-body">
						<div class="row">
							<div class="col-md-5 align-self-center">
								<h4 class="card-title">Print Address with Photo</h4>
							</div>
							<div class="col-md-7 align-self-center text-right">
								<div class="d-flex justify-content-end align-items-center">
						...
						<div class="m-t-20">
							<form name='resultForm' id="resultForm" class="form-horizontal" method='POST' action="" enctype="multipart/form-data">
								<div class="form-body">
									<div class="row">
										...
										<div class="col-md-3">
											<div class="row">
												<div class="col-md-7">
												<select class="form-control custom-select height40" id="language" name="language">
													<option value="1">English</option>
													<option value="2">Tamil</option>
												</select>
												<script>document.resultForm.language.value="<?php echo $_REQUEST['language']; ?>";</script>
												</div>
								...
								<div class="row">	
									<div class="col-md-12">
									  <div class="invoice multipage" id="print_content1" style="width:980px; margin:0 auto;">
										<?php
										if(isset($_REQUEST['data'])!=''){
											...
											if ($pdoRes->rowCount() > 0){											
												...
												while($obj=$pdoRes->fetch()){
													if($cityid != $obj->memb_city){
														$cityname .= $obj->city_name.', ';
														$cityid = $obj->memb_city;
													}													
													if($Sno == 0){
														echo '<div class="'.$break_class.'" width="100%" style="min-height:980px;">';
														//echo $header;
														echo '<table class="table table-bordered table-address-photo-'.$clms.'">';
														echo '<tr><th colspan="'.($clms+2).'">
													...
													if($_REQUEST['language'] == 1){
														...
														$memb_sname = '';
														if($obj->memb_sname != ''){
															$memb_sname = '<br>Spouse Name: '.$obj->memb_sname;
														}
														$memb_native = '';
														if($obj->memb_native > 0){
															$memb_native = '<br>Native City: '.$dbconn->GetSingleReconrd("mst_city","city_name","city_id",$obj->memb_native);
														}
														
														$memb_photo = '';
														if($obj->memb_photo != ''){
															$memb_photo = '<img src="photos/'.$obj->memb_photo.'" width="100%" alt="">';
														}														
														echo '<td width="'.($clmWidth-37).'%">'.$memb_photo.'</td><td class="height200" width="'.($clmWidth-13).'%"><span class="font-weight-bold">'.$obj->memb_fname.'. '.$obj->memb_initial.'</span><span class="memb-id">'.$obj->memb_code.'</span>'.$memb_lname.$memb_add1.$memb_add2.'																
																	<br>'.$obj->city_name.$memb_pincode.$memb_mobile.$memb_sname.$memb_native.'
																</td>';
													}else{
														$memb_lname = '';													
														if($obj->memb_lname != ''){
															$memb_lname = '<br>'.$obj->memb_lname_tamil;
														}
														$memb_add1 = '';
														if($obj->memb_add1 != ''){
															$memb_add1 = '<br>'.$obj->memb_add1_tamil;
														}
														$memb_add2 = '';
														if($obj->memb_add2 != ''){
															$memb_add2 = '<br>'.$obj->memb_add2_tamil;
														}
														$memb_pincode = '';
														if($obj->memb_pincode != ''){
															$memb_pincode = '-'.$obj->memb_pincode;
														...
														$memb_sname = '';
														if($obj->memb_sname_tamil != ''){
															$memb_sname = '<br>Spouse Name: '.$obj->memb_sname_tamil;
														}
														$memb_native = '';
														if($obj->memb_native > 0){
															$memb_native = '<br>Native City: '.$dbconn->GetSingleReconrd("mst_city","city_tamil","city_id",$obj->memb_native);
														}
														...
														echo '<td width="'.($clmWidth-37).'%">'.$memb_photo.'</td><td class="height200" width="'.($clmWidth-13).'%"><span class="font-weight-bold">'.$obj->memb_fname_tamil.'. '.$obj->memb_initial_tamil.'</span><span class="memb-id">'.$obj->memb_code.'</span>'.$memb_lname.$memb_add1.$memb_add2.'																
																	<br>'.$obj->city_tamil.$memb_pincode.$memb_mobile.$memb_sname.$memb_native.'
																</td>';
													}
													
													$Sno++;
													if ($Sno % $maxrows == 0 && $Sno > 0){
														echo '</tr></tbody>';
														echo '<tr><td colspan="'.($clms+2).'" align="center"><span class="font-weight-bold">'.substr($cityname, 0, -2).'</span></td></tr>';
														echo '</table></div>';
														$break_class = "page-break";
														$Sno = 0;
														$cityname = '';
														$cityid = 0;
													}													
		...
		$("#resultForm").on("submit", function(event) {
			...
			$.ajax({
				...
				success:function(result){
					//console.log(result);
					window.location.href = "address-photo-print.php?data="+result;
				}
			});
		});
